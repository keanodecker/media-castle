...
const Header = () => {
  ...
  const isActive = (path) => {
    if (path === '/#faq') return false; // Handled via hash if needed
    if (path === '/') return location.pathname === '/';
    return location.pathname.startsWith(path);
  };

  // Determine if we are on a dark hero section (Home or Partner page top)
  const isDarkHeroPage = location.pathname === '/' || location.pathname === '/partner';
  ...
  return (
    <header className="fixed top-4 left-0 right-0 z-50 px-4 sm:px-6 lg:px-8 transition-all duration-300">
      <div className="max-w-7xl mx-auto flex items-center gap-4 lg:gap-8">
        {/* Logo */}
        <Link to="/" className="flex-shrink-0 z-50 bg-white/10 backdrop-blur-md p-2 rounded-2xl border border-white/20">
          <img 
